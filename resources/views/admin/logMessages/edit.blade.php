...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.logMessage.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.log-messages.update", [$logMessage->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('log') ? 'is-invalid' : '' }}" name="log_id" id="log_id" required>
                    @foreach($logs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('log_id') ? old('log_id') : $logMessage->log->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('log'))
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\LogMessage::ROLE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('role') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="role_{{ $key }}" name="role" value="{{ $key }}" {{ old('role', $logMessage->role) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('message') ? 'is-invalid' : '' }}" name="message" id="message" required>{{ old('message', $logMessage->message) }}</textarea>
