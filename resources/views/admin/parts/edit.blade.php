    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.parts.update", [$part->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('data') ? 'is-invalid' : '' }}" name="data" id="data">{{ old('data', $part->data) }}</textarea>
                @if($errors->has('data'))
                    <div class="invalid-feedback">
                        {{ $errors->first('data') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Part::EXIST_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('exist') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="exist_{{ $key }}" name="exist" value="{{ $key }}" {{ old('exist', $part->exist) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('product_info') ? 'is-invalid' : '' }}" name="product_info" id="product_info">{{ old('product_info', $part->product_info) }}</textarea>
