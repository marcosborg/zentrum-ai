    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.instructions.update", [$instruction->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('text') ? 'is-invalid' : '' }}" name="text" id="text" required>{{ old('text', $instruction->text) }}</textarea>
                @if($errors->has('text'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('position') ? 'is-invalid' : '' }}" type="text" name="position" id="position" value="{{ old('position', $instruction->position) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="active" id="active" value="1" {{ $instruction->active || old('active', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('assistant') ? 'is-invalid' : '' }}" name="assistant_id" id="assistant_id">
                    @foreach($assistants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('assistant_id') ? old('assistant_id') : $instruction->assistant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
